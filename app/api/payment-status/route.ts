...
export async function GET(request: NextRequest) {  
  ...
  if (!pollUrl || !orderId) {  
    return NextResponse.json({ error: 'Missing poll_url or order_id' }, { status: 400 });  
  }  
  ...
  try {  
    ...
    // If payment is confirmed, update the database  
    if (status && status.status === 'paid') {  
      console.log(`[payment-status] Payment confirmed for order ${orderId}, updating database`);  
      ...
      if (updateError) {  
        ...
        console.log(`[payment-status] Order ${orderId} updated successfully`);  
      }  
    }  
